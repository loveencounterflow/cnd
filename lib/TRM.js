(function() {
  //###########################################################################################################
  var badge, color_code, color_name, effect_name, effect_names, effect_off, effect_on, get_timestamp, inspect, isa, lines_from_stdout, rainbow_color_names, rainbow_idx, ref, rpr, rpr_settings, type_of, σ_cnd;
  ...
  this._pen = function(...P) {
    ...
    R = (function() {
      ...
      for (i = 0, len = P.length; i < len; i++) {
        p = P[i];
        results.push(isa.text(p) ? p : this.rpr(p));
      }
      return results;
    }).call(this);
    ...
    ((effect_name, effect_on, effect_off) => {
      return this[effect_name] = (...P) => {
        ...
        for (idx = i = 0, len = P.length; i < len; idx = ++i) {
          p = P[idx];
          R.push(isa.text(p) ? p : this.rpr(p));
          if (idx !== last_idx) {
            R.push(effect_on);
  ...
  this._dir = function(x) {
    ...
    for (i = 0, len = ref1.length; i < len; i++) {
      ...
      for (j = 0, len1 = names.length; j < len1; j++) {
        name = names[j];
        marker = this._marker_from_type(type_of((Object.getOwnPropertyDescriptor(p, name))['value']));
